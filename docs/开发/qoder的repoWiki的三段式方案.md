...
### 关于阶段 B 中 `git add -f .qoder` 是否"污染历史"
...
git push origin cache/qoder-YYYYMMDD
```
...
- `.qoder/**` 的内容会进入 Git 对象库（`.git/objects`）
- 只要 `cache/qoder-YYYYMMDD` 分支存在，这个提交就一直存在于仓库历史中

#### 2. 但"污染"被隔离在快照分支，不影响主开发历史

- **main / feature 分支**：
  - `.qoder/` 在 `.gitignore` 中被忽略
  - 已经通过 `git rm -r --cached .qoder` 移除索引
  - 后续提交中不会再包含 `.qoder` 的内容

- **快照分支的隔离效果**：
  - 只在 `cache/qoder-*` 分支中使用 `git add -f .qoder`
  - 可以把"带缓存的大提交"锁在专用分支里
  - main 的提交历史仍然是干净的源码时间线
  - 功能分支的历史也不会出现 `.qoder` 相关变更

#### 3. 仓库体积会增加，但影响是可控的

**好处**：
- 可以把"生成成本高的 .qoder 缓存"安全备份到远程
- 避免误删后需要重新花钱生成

**代价**：
- 整体仓库对象体积会变大一些
- 特别是快照分支很多且 .qoder 很大时

**缓解方式**：
- 控制快照频率（例如重大更新或每月一次）
- 定期清理过期的 `cache/qoder-*` 分支（本地和远程）
- 协作者如果不需要 .qoder，可以只拉取 main/feature 分支
- 或使用浅克隆（`git clone --depth 1`）/过滤克隆减少下载体积

#### 4. 如果需要"绝对不把 .qoder 写进任何历史"的替代方案

- 可以使用单独仓库存放 .qoder
- 或上传到对象存储（OSS）/ 网盘
- 当前三段式方案是在"**不污染主开发分支 + 保留 Git 备份能力**"之间的折中选择
- 更适合日常开发和成本优化场景

**结论**：虽然技术上会写入 Git 历史，但通过分支隔离策略，确保了主开发历史的干净，是可接受的权衡。
